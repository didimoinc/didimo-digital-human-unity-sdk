...
internal abstract class DidimoBufferedLinearInterpolator<T> where T : struct
{
    ...
    private void TryConsumeFromBuffer(double renderTime, double serverTime)
    {
        ...
        if (renderTime >= m_EndTimeConsumed)
        {
            ...
            for (int i = m_Buffer.Count - 1;
                 i >= 0;
                 i--) // todo stretch: consume ahead if we see we're missing values due to packet loss
            {
    ...
    public T Update(float deltaTime, double renderTime, double serverTime)
    {
        ...
        if (m_LifetimeConsumedCount >=
            1) // shouldn't interpolate between default values, let's wait to receive data first, should only interpolate between real measurements
        {
            float t = 1.0f;
            ...
            if (range > k_SmallValue)
            {
                ...
                if (t < 0.0f)
                {
                    throw new OverflowException(
                        $"t = {t} but must be >= 0. range {range}, RenderTime {renderTime}, Start time {m_StartTimeConsumed}, end time {m_EndTimeConsumed}");
                }
            ...
            m_CurrentInterpValue =
                Interpolate(m_CurrentInterpValue, target,
                    deltaTime / maxInterpTime); // second interpolate to smooth out extrapolation jumps
        }
    ...
    public void AddMeasurement(T newMeasurement, double sentTime)
    {
        ...
        if (sentTime > m_EndTimeConsumed ||
            m_LifetimeConsumedCount == 0) // treat only if value is newer than the one being interpolated to right now
        {
            m_LastBufferedItemReceived = new BufferedItem(newMeasurement, sentTime);
...
internal class DidimoBufferedLinearInterpolatorFloat : DidimoBufferedLinearInterpolator<float>
{
...
internal class DidimoBufferedLinearInterpolatorQuaternion : DidimoBufferedLinearInterpolator<Quaternion>
{
