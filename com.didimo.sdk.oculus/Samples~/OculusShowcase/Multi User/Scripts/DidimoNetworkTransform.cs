...
public class DidimoNetworkTransform : NetworkBehaviour
{
    ...
    public delegate (Vector3 pos, Quaternion rotOut, Vector3 scale) OnClientRequestChangeDelegate(Vector3 pos,
    ...
    private readonly NetworkVariable<NetworkTransformState> m_ReplicatedNetworkState =
        new NetworkVariable<NetworkTransformState>(new NetworkTransformState());
    ...
    private bool
        m_HasSentLastValue =
            false; // used to send one last value, so clients can make the difference between lost replication data (clients extrapolate) and no more data to send.


    private DidimoBufferedLinearInterpolator<float> m_PositionXInterpolator; // = new BufferedLinearInterpolatorFloat();
    private DidimoBufferedLinearInterpolator<float> m_PositionYInterpolator; // = new BufferedLinearInterpolatorFloat();
    private DidimoBufferedLinearInterpolator<float> m_PositionZInterpolator; // = new BufferedLinearInterpolatorFloat();

    private DidimoBufferedLinearInterpolator<Quaternion>
    ...
    private DidimoBufferedLinearInterpolator<float> m_ScaleXInterpolator; // = new BufferedLinearInterpolatorFloat();
    private DidimoBufferedLinearInterpolator<float> m_ScaleYInterpolator; // = new BufferedLinearInterpolatorFloat();
    private DidimoBufferedLinearInterpolator<float> m_ScaleZInterpolator; // = new BufferedLinearInterpolatorFloat();

    private readonly List<DidimoBufferedLinearInterpolator<float>> m_AllFloatInterpolators =
        new List<DidimoBufferedLinearInterpolator<float>>(6);

    protected Transform
        m_Transform; // cache the transform component to reduce unnecessary bounce between managed and native
    ...
    {
        var isDirty = ApplyTransformToNetworkStateWithInfo(ref m_LocalAuthoritativeNetworkState, dirtyTime, position,
            rotation, scale);
    ...
    private void TryCommit(bool isDirty)
    {
        ...
        if (isDirty)
        {
            ...
            (!m_HasSentLastValue &&
             m_CachedNetworkManager.LocalTime.Tick >=
             m_LastSentTick +
             1) // check for state.IsDirty since update can happen more than once per tick. No need for client, RPCs will just queue up
        {
    ...
    {
        if (serverParams.Receive.SenderClientId ==
            OwnerClientId) // RPC call when not authorized to write could happen during the RTT interval during which a server's ownership change hasn't reached the client yet
        {
    ...
    internal bool ApplyTransformToNetworkState(ref NetworkTransformState networkState, double dirtyTime,
    ...
    private (bool isDirty, bool isPositionDirty, bool isRotationDirty, bool isScaleDirty)
        ApplyTransformToNetworkStateWithInfo(ref NetworkTransformState networkState, double dirtyTime,
            Transform transformToUse)
    {
    ...
    private (bool isDirty, bool isPositionDirty, bool isRotationDirty, bool isScaleDirty)
        ApplyTransformToNetworkStateWithInfo(ref NetworkTransformState networkState, double dirtyTime, Vector3 position,
            Vector3 rotAngles, Vector3 scale)
    {
        var isDirty = false;
    ...
    private void ApplyInterpolatedNetworkStateToTransform(NetworkTransformState networkState,
        Transform transformToUpdate)
    {
        ...
        if (SyncPositionX)
        {
            interpolatedPosition.x = networkState.IsTeleportingNextFrame || !Interpolate
                ? networkState.Position.x
                : m_PositionXInterpolator.GetInterpolatedValue();
        }

        if (SyncPositionY)
        {
            interpolatedPosition.y = networkState.IsTeleportingNextFrame || !Interpolate
                ? networkState.Position.y
                : m_PositionYInterpolator.GetInterpolatedValue();
        }

        if (SyncPositionZ)
        {
            interpolatedPosition.z = networkState.IsTeleportingNextFrame || !Interpolate
                ? networkState.Position.z
                : m_PositionZInterpolator.GetInterpolatedValue();
        }
        ...
        if (SyncRotAngleX || SyncRotAngleY || SyncRotAngleZ)
        {
            ...
            if (SyncRotAngleX)
            {
                interpolatedRotAngles.x = networkState.IsTeleportingNextFrame || !Interpolate
                    ? networkState.Rotation.x
                    : eulerAngles.x;
            }

            if (SyncRotAngleY)
            {
                interpolatedRotAngles.y = networkState.IsTeleportingNextFrame || !Interpolate
                    ? networkState.Rotation.y
                    : eulerAngles.y;
            }

            if (SyncRotAngleZ)
            {
                interpolatedRotAngles.z = networkState.IsTeleportingNextFrame || !Interpolate
                    ? networkState.Rotation.z
                    : eulerAngles.z;
            }
        }
        ...
        if (SyncScaleX)
        {
            interpolatedScale.x = networkState.IsTeleportingNextFrame || !Interpolate
                ? networkState.Scale.x
                : m_ScaleXInterpolator.GetInterpolatedValue();
        }

        if (SyncScaleY)
        {
            interpolatedScale.y = networkState.IsTeleportingNextFrame || !Interpolate
                ? networkState.Scale.y
                : m_ScaleYInterpolator.GetInterpolatedValue();
        }

        if (SyncScaleZ)
        {
            interpolatedScale.z = networkState.IsTeleportingNextFrame || !Interpolate
                ? networkState.Scale.z
                : m_ScaleZInterpolator.GetInterpolatedValue();
        }
    ...
    private void OnNetworkStateChanged(NetworkTransformState oldState, NetworkTransformState newState)
    {
        ...
        // Debug.Log($"OnNetworkStateChanged for object {gameObject.name}: {newState.ToString()}");
        ...
        //     Debug.DrawLine(pos, pos + Vector3.up + Vector3.left * Random.Range(0.5f, 2f), Color.green,
        //         k_DebugDrawLineTime, false);
        // }
    }
    ...
    private void Awake()
    {
        ...
        m_PositionXInterpolator = new DidimoBufferedLinearInterpolatorFloat();
        m_PositionYInterpolator = new DidimoBufferedLinearInterpolatorFloat();
        m_PositionZInterpolator = new DidimoBufferedLinearInterpolatorFloat();
        m_RotationInterpolator =
            new DidimoBufferedLinearInterpolatorQuaternion(); // rotation is a single Quaternion since each euler axis will affect the quaternion's final value
        m_ScaleXInterpolator = new DidimoBufferedLinearInterpolatorFloat();
        m_ScaleYInterpolator = new DidimoBufferedLinearInterpolatorFloat();
        m_ScaleZInterpolator = new DidimoBufferedLinearInterpolatorFloat();
    ...
    public override void OnNetworkSpawn()
    {
        ...
        if (CanCommitToTransform)
        {
            // WHY DOESN'T THIS WORK?? WHY DOESN'T IT SET INITIAL TRANSFORM CORRECTLY?
    ...
    public void SetState(Vector3? posIn = null, Quaternion? rotIn = null, Vector3? scaleIn = null,
        bool shouldGhostsInterpolate = true)
    {
        if (!IsOwner)
        {
 ...
 new Vector3(m_PositionXInterpolator.GetInterpolatedValue(), m_PositionYInterpolator.GetInterpolatedValue(), m_PositionZInterpolator.GetInterpolatedValue());
                        ...
                        var oldStateDirtyInfo =
 ApplyTransformToNetworkStateWithInfo(ref m_PrevNetworkState, 0, m_Transform);
                        ...
                        if (oldStateDirtyInfo.isPositionDirty || oldStateDirtyInfo.isScaleDirty || (oldStateDirtyInfo.isRotationDirty && SyncRotAngleX && SyncRotAngleY && SyncRotAngleZ))
                        {
                            ...
                            var dirtyField =
 oldStateDirtyInfo.isPositionDirty ? "position" : oldStateDirtyInfo.isRotationDirty ? "rotation" : "scale";
