...
namespace Didimo.Core.Deformables
{
    public class ObjDeformationUtility : DeformationUtility
    {
        ...
        public override byte[] Serialize()
        {
            ...
            {
                stringBuilder.Append($"v {vertex[0].ToString(cultureInfo)} " +
                    $"{vertex[1].ToString(cultureInfo)} {vertex[2].ToString(cultureInfo)}\n");
            }

            foreach (Vector3 normal in normals)
            {
                stringBuilder.Append($"vn {normal[0].ToString(cultureInfo)} " +
                    $"{normal[1].ToString(cultureInfo)} {normal[2].ToString(cultureInfo)}\n");
            }
            ...
            foreach (Vector3Int face in faces)
            {
                ...
                stringBuilder.Append($"f {vertexIdX}/{vertexIdX}/{vertexIdX} " +
                    $"{vertexIdY}/{vertexIdY}/{vertexIdY} {vertexIdZ}/{vertexIdZ}/{vertexIdZ}\n");
            }
        ...
        {
            return new Vector2(float.Parse(lineSplit[1], CultureInfo.InvariantCulture),
                float.Parse(lineSplit[2], CultureInfo.InvariantCulture));
        }
        ...
        public override void FromMeshFilters(MeshFilter[] meshFilters, bool shared = true)
        {
            ...
            foreach (MeshFilter meshFilter in meshFilters)
            {
                ...
                for (int i = 0; i < mesh.subMeshCount; i++)
                {
                    ...
                    {
                        faces.Add(new Vector3Int(indices[(j * 3) + 2] + indexOffset,
                            indices[(j * 3) + 1] + indexOffset, indices[j * 3] + indexOffset));
                    }
                }
