	...
	[Preserve] public class GLTFMaterial {
		...
		public IEnumerator CreateMaterial(GLTFTexture.ImportResult[] textures, ShaderSettings shaderSettings, Action<Material> onFinish, Func<string, Shader> shaderForName) {
			...
			if (normalTexture != null) {
				en = TryGetTexture(textures, normalTexture, true, true, tex => {
					if (tex != null) {
			...
			if (occlusionTexture != null) {
				en = TryGetTexture(textures, occlusionTexture, true, false, tex => {
					if (tex != null) {
			...
			if (emissiveTexture != null) {
				en = TryGetTexture(textures, emissiveTexture, false, false, tex => {
					if (tex != null) {
		...
		public static IEnumerator TryGetTexture(GLTFTexture.ImportResult[] textures, TextureInfo texture, bool linear, bool normalMap, Action<Texture2D> onFinish, Action<float> onProgress = null) {
		...
		[Preserve] public class PbrMetalRoughness {
			...
			private IEnumerator TrySetTexture(GLTFTexture.ImportResult[] textures, TextureInfo textureInfo, Material mat, string shaderProperty, bool linear=false, bool normalMap=false) {
				if (textureInfo == null) {
					yield break;
				}
			...
			public IEnumerator CreateMaterial(GLTFTexture.ImportResult[] textures, AlphaMode alphaMode, Extensions extensions, ShaderSettings shaderSettings, Action<Material> onFinish, Func<string, Shader> shaderForName) {
				// Shader
				Shader sh = null;
				bool didimoExtension = extensions?.didimo != null;
				// Load didimo shader
				...
				if (!didimoExtension) {
					...
					IEnumerator en = TrySetTexture(textures, baseColorTexture, mat, "_BaseMap", false, false);
					while (en.MoveNext()) { yield return null; }

					if (matProperties != null) {

						en = TrySetTexture(textures, matProperties.transTexture, mat, "_TransMap", true, false);
						while (en.MoveNext()) { yield return null; }
				...
				if (textures != null) {
					...
					if (diffuseTexture != null) {
						if (textures.Length <= diffuseTexture.index) {
							...
							IEnumerator en = textures[diffuseTexture.index].GetTextureCached(false, false, tex => {
								if (tex != null) {
									mat.SetTexture("_MainTex", tex);
					...
					if (specularGlossinessTexture != null) {
						if (textures.Length <= specularGlossinessTexture.index) {
							...
							IEnumerator en = textures[specularGlossinessTexture.index].GetTextureCached(false, false, tex => {
								if (tex != null) {
