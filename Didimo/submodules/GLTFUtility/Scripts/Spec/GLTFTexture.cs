...
namespace Didimo.GLTFUtility {
	...
	[Preserve] public class GLTFTexture {
		...
		public class ImportResult {
			...
			public IEnumerator GetTextureCached(bool linear, bool normalMap, Action<Texture2D> onFinish, Action<float> onProgress = null) {
				if (cache == null) {
					IEnumerator en = image.CreateTextureAsync(linear, normalMap, x => cache = x, onProgress);
