...
namespace Didimo.Core.Editor
{
    public static class EditorMaterialUtility
    {
        ...
        static Dictionary<EBodyPartID, String[]> gMaterialNameToMergedName = new Dictionary<EBodyPartID, String[]>()
        {
            { EBodyPartID.BODY, gBodySkinMaterialNames},
            { EBodyPartID.HEAD, gSkinMaterialNames}
        };
        ...
        public static bool IsBodyPartAtlasable(EBodyPartID bodyPart)
        {
            return bodyPart == EBodyPartID.BODY || bodyPart == EBodyPartID.HEAD;
        }

        public static Renderer[] FindAppropriateRenderList(GameObject go, EBodyPartID bodyPartID)
        {
        ...
        public static void GenerateMergedAndAtlasedTextureMaterialsForSelected(GameObject[] objects, int materialSwitcherSlot = DidimoMaterialSwitcher.COMBINED_ATLASED_MATERIAL_SET, bool numberAtlasedTextures = false)
        {
            ...
            System.ValueTuple<EBodyPartID, String> []atlasedMaterialNames = gMaterialNameToMergedName.Select(s => new System.ValueTuple<EBodyPartID, String>(s.Key, s.Value[1])).ToArray();
            ...
            for (var matIdx = 0; matIdx < atlasedMaterialNames.Length; ++matIdx)
            {
                ...
                if (matGUIDS.Length > 0)
                {
                    ...
                    if (mergedAtlasedMaterial)
                    {
                        ...
                        {
                            var renderlist = FindAppropriateRenderList(go, materialID);
                        ...
                        var baseMap = EditorTextureUtility.CreateSerialisedTextureAtlas(altasMaterialSlots, "_BaseMap", new Vector2Int(4096, 4096), new Vector2Int(4, 4), gAtlasedTextureDirectory + "DiffuseTextureAtlas.png", baseFlags | TextureUtility.PROC_CREATE_EVEN_IF_EXISTS | TextureUtility.PROC_LINEAR_COLOURSPACE, TextureUtility.MODE_LINEAR_TO_SRGB);
                        var normalMap = EditorTextureUtility.CreateSerialisedTextureAtlas(altasMaterialSlots, "_NormalMap", new Vector2Int(4096, 4096), new Vector2Int(4, 4), gAtlasedTextureDirectory +"NormalTextureAtlas.png", baseFlags | TextureUtility.PROC_CREATE_EVEN_IF_EXISTS | TextureUtility.PROC_LINEAR_COLOURSPACE, TextureUtility.MODE_RG_NORMAL_TO_XYZ);
                        var RPCAOMap = EditorTextureUtility.CreateSerialisedTextureAtlas(altasMaterialSlots, "_Rough_Spec_Cavity_AO", new Vector2Int(4096, 4096), new Vector2Int(4, 4), gAtlasedTextureDirectory + "Rough_Spec_Cavity_AOTextureAtlas.png", baseFlags | TextureUtility.PROC_CREATE_EVEN_IF_EXISTS | TextureUtility.PROC_LINEAR_COLOURSPACE | TextureUtility.PROC_LOAD_ORIGINAL_IMAGE, TextureUtility.MODE_LINEAR_TO_SRGB);
        ...
        static String GetAppropriatePrefixNameFromMaterials(Material[] materials, int level)
        {
            foreach (var m in materials)
            {
                var bodyPartID = GetBodyPartID(m.name);                
        ...
        public static void AssignMergedMaterialPropertiesFromSeparated(Material sourceMaterial, Material targetMaterial, string newMaterialPath, string newTexturePrefix, bool forceCreateTextures)
        {
            ...
            for (var i = 0; i < gTextureMergeSpecs.Length; ++i)
            {
                ...
                if (targetMaterial.HasProperty(mspec.MaterialPropName))
                {
                    ...
                    AssetDatabase.Refresh();
#endif
                    Texture newTexture = EditorTextureUtility.CreateSerialisedMergedTextureFromMaterial(sourceMaterial, fname, mspec.SubChannelMapNames, TextureUtility.PROC_LINEAR_COLOURSPACE | (forceCreateTextures ? TextureUtility.PROC_CREATE_EVEN_IF_EXISTS : 0));
        ...
        public static void GenerateMergedTextureMaterialsForSelected(GameObject[] objects, int materialSwitcherSlot = DidimoMaterialSwitcher.COMBINED_MATERIAL_SET, bool forceCreateMaterial = false, bool forceCreateTextures = false)
        {
            ...
            MaterialUtility.EnsureMaterialSwitcher(objects, true); //Ensures material switchers (and, if a group is selected, material switcher global index controller) are added to appropriate mesh renderers
            foreach (GameObject go in objects)
            {
                try
                {
