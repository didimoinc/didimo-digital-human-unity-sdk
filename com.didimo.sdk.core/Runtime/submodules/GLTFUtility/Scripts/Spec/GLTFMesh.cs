...
#region Import
		...
		public class ImportTask : Importer.ImportTask<ImportResult[]> {
			...
			ImportSettings.ModelImporterNormalsTangents normalsImportMode;
			ImportSettings.ModelImporterNormalsTangents tangentsImportMode;
			private class MeshData {
				...
				public Mesh ToMesh(ImportSettings.ModelImporterNormalsTangents normalsImportMode, ImportSettings.ModelImporterNormalsTangents tangentsImportmode) {
					Mesh mesh = new Mesh();
					...
					switch (normalsImportMode) {
						case ImportSettings.ModelImporterNormalsTangents.Calculate:
							if (!onlyTriangles) {
								Debug.LogError($"Mesh '{name}' has non-triangle polygons. Will not calculate normals.");
							}
							else {
								mesh.RecalculateNormals();
							}
							break;
						case ImportSettings.ModelImporterNormalsTangents.Import:
							if (normals.Count == 0) mesh.RecalculateNormals();
							else mesh.normals = normals.ToArray();
							break;
						case ImportSettings.ModelImporterNormalsTangents.None:
							mesh.SetNormals(new List<Vector3>());
							break;
					}

					switch (tangentsImportmode) {
						case ImportSettings.ModelImporterNormalsTangents.Calculate:
							if (!onlyTriangles) {
								Debug.LogError($"Mesh '{name}' has non-triangle polygons. Will not calculate tangents.");
							}
							else {
						...
						case ImportSettings.ModelImporterNormalsTangents.Import:
							...
							else mesh.SetTangents(tangents.ToArray());
							break;
						case ImportSettings.ModelImporterNormalsTangents.None:
			...
			public ImportTask(List<GLTFMesh> meshes, GLTFAccessor.ImportTask accessorTask, GLTFBufferView.ImportTask bufferViewTask, GLTFMaterial.ImportTask materialTask, ImportSettings importSettings) : base(accessorTask, materialTask) {
				...
				normalsImportMode = importSettings.normals;
				tangentsImportMode = importSettings.tangents;
			...
			public override IEnumerator OnCoroutine(Action<float> onProgress = null) {
				...
				for (int i = 0; i < meshData.Length; i++) {
					...
					Result[i].mesh = meshData[i].ToMesh(normalsImportMode, tangentsImportMode);
