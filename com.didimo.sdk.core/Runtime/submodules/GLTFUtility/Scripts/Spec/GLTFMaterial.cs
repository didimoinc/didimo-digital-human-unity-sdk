...
namespace Didimo.GLTFUtility
{
	...
	public class GLTFMaterial
	{
		...
		{
			get { return _defaultMaterial != null ? _defaultMaterial : _defaultMaterial = UnityEditor.AssetDatabase.GetBuiltinExtraResource<Material>("Default-Material.mat"); }
		}
		...
		public IEnumerator CreateMaterial(GLTFTexture.ImportResult[] textures, ShaderSettings shaderSettings, Action<Material> onFinish, Func<string, Shader> shaderForName)
		{
			Material mat = null;
			IEnumerator en = null;
			...
			{
				en = pbrMetallicRoughness.CreateMaterial(textures, alphaMode, extensions, shaderSettings, x => mat = x, shaderForName);
			...
			bool didimoExtension = extensions?.didimo != null;

			// Normal texture
			if (normalTexture != null)
			{
				en = TryGetTexture(textures,
					normalTexture,
					true,
					true,
					tex =>
					{
						if (tex != null)
						{
							mat.SetTexture(didimoExtension ? "_NormalMap" : "_BumpMap", tex);
			...
			if (occlusionTexture != null)
			{
				en = TryGetTexture(textures,
					...
					tex =>
					{
						if (tex != null)
						{
							mat.SetTexture(didimoExtension ? "_AmbientOcclusionMap" : "_OcclusionMap", tex);
		...
		public static IEnumerator TryGetTexture(GLTFTexture.ImportResult[] textures, TextureInfo texture, bool linear, bool normalMap, Action<Texture2D> onFinish,
			Action<float> onProgress = null)
		{
			...
			IEnumerator en = textures[texture.index].GetTextureCached(linear, normalMap, onFinish, onProgress);
			while (en.MoveNext()) { yield return null; }
		...
		public class Extensions
		{
			...
			public DidimoExtension       didimo                              = null;
		}
		...
		public class DidimoExtension
		{
			...
			public DidimoMaterialProperties materialProperties;
		}

		[Preserve]
		public class DidimoMaterialProperties
		{
			public TextureInfo transTexture;
			public TextureInfo specTexture;
			public TextureInfo roughTexture;
			public TextureInfo cavityTexture;
			public TextureInfo zBiasTexture;
			public TextureInfo microNormal;
			public TextureInfo opacityTexture;
			public TextureInfo sssAoMask;
		...
		public class PbrMetalRoughness
		{
			...
			private IEnumerator TrySetTexture(GLTFTexture.ImportResult[] textures, TextureInfo textureInfo, Material mat, string shaderProperty, bool linear = false,
				bool normalMap = false)
			{
				...
				IEnumerator en = textures[textureInfo.index]
					.GetTextureCached(linear,
						normalMap,
						tex =>
						{
							if (tex != null)
							{
								if (mat.HasProperty(shaderProperty)) mat.SetTexture(shaderProperty, tex);
							}
						});
				while (en.MoveNext()) { yield return null; }
			}

			public IEnumerator CreateMaterial(GLTFTexture.ImportResult[] textures, AlphaMode alphaMode, Extensions extensions, ShaderSettings shaderSettings,
				Action<Material> onFinish, Func<string, Shader> shaderForName)
			{
				// Shader
				Shader sh = null;
				bool didimoExtension = extensions?.didimo != null;
				// Load didimo shader
				// TODO: Get the shader for the current render pipeline
				if (didimoExtension)
				{
					sh = shaderForName(extensions.didimo.shader);
				}
				else if (alphaMode == AlphaMode.BLEND) sh = shaderSettings.MetallicBlend;
				...
				string albedoPropertyName = didimoExtension ? "_BaseMap" : "_MainTex";
				...
				if (textures != null)
				{
					...
					if (baseColorTexture != null && baseColorTexture.index >= 0)
					{
						...
						{
							IEnumerator en = textures[baseColorTexture.index]
								.GetTextureCached(false,
									false,
									tex =>
									{
										if (tex != null)
										{
											mat.SetTexture(albedoPropertyName, tex);
											if (baseColorTexture.extensions != null)
											{
												baseColorTexture.extensions.Apply(baseColorTexture, mat, albedoPropertyName);
											}
										}
									});
							while (en.MoveNext()) { yield return null; }
					...
					if (metallicRoughnessTexture != null && metallicRoughnessTexture.index >= 0)
					{
						...
						{
							IEnumerator en = TryGetTexture(textures,
				...
				if (didimoExtension)
				{
					DidimoMaterialProperties matProperties = extensions.didimo.materialProperties;

					if (matProperties != null)
					{
						IEnumerator en = TrySetTexture(textures, matProperties.transTexture, mat, "_TransMap", true, false);
						while (en.MoveNext()) { yield return null; }

						en = TrySetTexture(textures, matProperties.specTexture, mat, "_SpecularMap", true, false);
						while (en.MoveNext()) { yield return null; }

						en = TrySetTexture(textures, matProperties.roughTexture, mat, "_RoughnessMap", true, false);
						while (en.MoveNext()) { yield return null; }

						en = TrySetTexture(textures, matProperties.cavityTexture, mat, "_CavityMap", true, false);
						while (en.MoveNext()) { yield return null; }

						en = TrySetTexture(textures, matProperties.zBiasTexture, mat, "_ZBiasMap", true, false);
						while (en.MoveNext()) { yield return null; }

						en = TrySetTexture(textures, matProperties.microNormal, mat, "_NormalMicro", true, true);
						while (en.MoveNext()) { yield return null; }

						en = TrySetTexture(textures, matProperties.opacityTexture, mat, "_OpacityMap", true, false);
						while (en.MoveNext()) { yield return null; }

						en = TrySetTexture(textures, matProperties.sssAoMask, mat, "_SssAoMask", true, false);
						while (en.MoveNext()) { yield return null; }

						en = TrySetTexture(textures, matProperties.alphaMask, mat, "_AlphaMask", true, false);
						while (en.MoveNext()) { yield return null; }

						if (mat.HasProperty("_ColorTint")) mat.SetColor("_ColorTint", baseColorFactor);
			...
			public IEnumerator CreateMaterial(GLTFTexture.ImportResult[] textures, AlphaMode alphaMode, ShaderSettings shaderSettings, Action<Material> onFinish)
			{
				...
				if (textures != null)
				{
					...
					if (diffuseTexture != null)
					{
						...
						{
							IEnumerator en = textures[diffuseTexture.index]
								.GetTextureCached(false,
									false,
									tex =>
									{
					...
					if (specularGlossinessTexture != null)
					{
						...
						{
							Debug.LogWarning("Attempted to get specularGlossinessTexture texture index " +
						...
						else
						{
							...
							IEnumerator en = textures[specularGlossinessTexture.index]
								.GetTextureCached(false,
									false,
									tex =>
									{
		...
		public class ImportTask : Importer.ImportTask<ImportResult[]>
		{
			...
			public ImportTask(List<GLTFMaterial> materials, GLTFTexture.ImportTask textureTask, ImportSettings importSettings) : base(textureTask)
			{
				...
				task = new Task(() =>
				{
					if (materials == null || !importSettings.materials) return;
