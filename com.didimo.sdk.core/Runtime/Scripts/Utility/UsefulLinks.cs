namespace Didimo.Core.Utility
{
    public static class UsefulLinks
    {
        public static readonly string CUSTOMER_PORTAL = "https://app.didimo.co";
        public static readonly string DEVELOPER_PORTAL = "https://didimo.readme.io/docs";
        public static readonly string CREATING_A_DIDIMO_DEVELOPER_PORTAL
            = "https://didimo.readme.io/docs/creating-a-didimo";
        public static readonly string ARKIT_GUIDE = "https://bit.ly/2ZBH6rR";
        public static readonly string PIPELINE_DOCUMENTATION = "https://docs.unity3d.com/Manual/render-pipelines.html";
        public static readonly string URP_DOCUMENTATION = "https://docs.unity3d.com/Packages/com.unity.render-pipelines.universal@11.0/manual/";
        public static readonly string HDRP_DOCUMENTATION = "https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@14.0/manual/index.html";
        public static readonly string BIRP_DOCUMENTATION = "https://docs.unity3d.com/Manual/built-in-render-pipeline.html";

    }
}