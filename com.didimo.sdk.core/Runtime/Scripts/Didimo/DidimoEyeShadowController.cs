...
namespace Didimo
{
    ...
    public class DidimoEyeShadowController : DidimoBehaviour
    {
        ...
        public static class DidimoEyeShadowSettings
        {
        ...
        private static Transform[] SetTransforms(GameObject go, string[] transformNames)
        {
            ...
            for (int i = 0; i < transformNames.Length; i++)
                transforms[i] = ComponentUtility.GetChildWithName(go, transformNames[i], true).GetComponent<Transform>();
            return transforms;
        }

        public void BuildFromComponents()
        {
            HeadTransform = ComponentUtility.GetChildWithName(gameObject, DidimoEyeShadowSettings.headJoint, true).GetComponent<Transform>();
            LeftEye = ComponentUtility.GetChildWithName(gameObject, DidimoEyeShadowSettings.leftEyeMesh, true).GetComponent<SkinnedMeshRenderer>();
            RightEye = ComponentUtility.GetChildWithName(gameObject, DidimoEyeShadowSettings.rightEyeMesh, true).GetComponent<SkinnedMeshRenderer>();
            if (HeadTransform == null)
                HeadTransform = gameObject.transform;
            LeftEyeHoleBoneTransforms = SetTransforms(gameObject, DidimoEyeShadowSettings.leftEyelidJoints);
            RightEyeHoleBoneTransforms = SetTransforms(gameObject, DidimoEyeShadowSettings.rightEyelidJoints);
            AOpowVarNameID = -1;
            initialised = true;
        }
        ...
        if (LeftEye == null)
            LeftEye = GameObject.Find("mesh_l_low_eye_001")?.GetComponent<SkinnedMeshRenderer>();
        if (RightEye == null)
            RightEye = GameObject.Find("mesh_r_low_eye_001")?.GetComponent<SkinnedMeshRenderer>();*/
